...
public class PlayerListAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case ITEM_TYPE_VIDEO:
                viewHolder = new VideoViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_video, parent, false));
                break;
            case ITEM_TYPE_AUDIO:
                viewHolder = new AudioViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_audio, parent, false));
                break;
            case ITEM_TYPE_TEXT:
                viewHolder = new TextViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_text, parent, false));
                break;
            case ITEM_TYPE_SCROLL_LIST:
                viewHolder = new ScrollListViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_scroll_list, parent, false));
                break;
            case ITEM_TYPE_IMAGE:
                viewHolder = new ImageViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_image, parent, false));
                break;
    ...
    public void onBindViewHolder(BaseViewHolder holder, final int position) {
        ...
        } else if (itemViewType == ITEM_TYPE_SCROLL_LIST) {
            if(holder instanceof ScrollListViewHolder){
                ScrollListViewHolder scrollListViewHolder = (ScrollListViewHolder) holder;
                List<BaseData> data = (List<BaseData>) mDatas.get(position).data;
                scrollListViewHolder.setDatas(data);
            }
